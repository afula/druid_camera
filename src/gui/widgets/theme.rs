pub use druid::theme::*;
use druid::{
	widget::prelude::*, Color, Env, FontDescriptor, FontFamily, FontWeight, Insets, Key, Size,
};

use crate::gui::data::{AppState, Theme};
...
impl<W: Widget<AppState>> Widget<AppState> for ThemeScope<W> {
	fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut AppState, env: &Env) {
		self.inner.event(ctx, event, data, self.cached_env.as_ref().unwrap_or(env))
	}
	...
	fn lifecycle(&mut self, ctx: &mut LifeCycleCtx, event: &LifeCycle, data: &AppState, env: &Env) {
		...
		self.inner.lifecycle(ctx, event, data, self.cached_env.as_ref().unwrap_or(env))
	}
	...
	fn update(&mut self, ctx: &mut UpdateCtx, old_data: &AppState, data: &AppState, env: &Env) {
		...
		self.inner.update(ctx, old_data, data, self.cached_env.as_ref().unwrap_or(env));
	}
	...
	fn layout(
		...
		self.inner.layout(ctx, bc, data, self.cached_env.as_ref().unwrap_or(env))
	}

	fn paint(&mut self, ctx: &mut PaintCtx, data: &AppState, env: &Env) {
		self.inner.paint(ctx, data, self.cached_env.as_ref().unwrap_or(env));
	}
}
...
fn setup(env: &mut Env, state: &AppState) {
	match state.theme {
	...
	env.set(
		UI_FONT_MEDIUM,
		FontDescriptor::new(FontFamily::SYSTEM_UI).with_size(13.0).with_weight(FontWeight::MEDIUM),
	);
